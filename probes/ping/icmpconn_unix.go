...
// We need to write our own listenPacket instead of using the net.ListenPacket
// for the following reasons:
//   1. ListenPacket doesn't support ICMP for SOCK_DGRAM sockets. You create
//      datagram sockets by specifying network as "udp", but the implementation
//      ignores the protocol field for "udp" network.
//   2. ListenPacket doesn't support setting up socket options (we need
...
func (ipc *icmpPacketConn) read(buf []byte) (n int, addr net.Addr, fetchTime time.Time, err error) {
	...
	for _, m := range cmsgs {
		...
		if m.Header.Type == syscall.SCM_TIMESTAMP {
			...
			binary.Read(bytes.NewReader(m.Data), binary.LittleEndian, &tv)
			fetchTime = time.Unix(tv.Unix()) // syscall.Timeval -> time.Time
		}
	}
