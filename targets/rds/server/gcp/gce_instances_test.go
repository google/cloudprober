...
func TestInstancesResources(t *testing.T) {
	...
	// #################################################################
	// Instances, with first NIC and private IP
	// #################################################################
	testListResources(t, nil, nil, lister, testInstances, testIndex, "private")
	...
	testListResources(t, nil, ipConfig, lister, testInstances, testIndex, "public")
	...
	testListResources(t, nil, ipConfig, lister, testInstances, testIndex, "ipAliasRange")
	...
	testListResources(t, nil, ipConfig, lister, testInstances[1:], testIndex, "private")

	// #################################################################
	// Instances, with second NIC and public IP
	// We get no instances in results as first one doesn't have second NIC
	// and second instance's second NIC doesn't have a public IP.
	// ##################################################################
	testIndex = 1
	ipConfig = &pb.IPConfig{
		NicIndex: proto.Int32(int32(testIndex)),
	...
	testListResources(t, nil, ipConfig, lister, testInstances[2:], testIndex, "public")
}

func testListResources(t *testing.T, filter *pb.Filter, ipConfig *pb.IPConfig, gil *gceInstancesLister, testInstances []*testInstance, testIndex int, ipTypeStr string) {
	resources := gil.listResources(filter, ipConfig)
	if len(resources) != len(testInstances) {
		t.Errorf("Got wrong number of targets. Expected: %d, Got: %d", len(testInstances), len(resources))
	}
